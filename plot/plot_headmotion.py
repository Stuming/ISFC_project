...
def plot_headmotion(mcdat, fig_show=True, save_path=None, title_id=None, update=False):
    """
    Plot head motion, and save figure into file(optional).
    ...
    Notes
    -----
        fmcpr.mcdat file is the motion estimates(mm and degrees).
...
def get_para(mcdat, index):
    ...
    ----------
        mcdat: load file that contains motion estimates. Like: fmcpr.mcdat
        index: decide which row of mcdat should be returned.

    Returns
    -------
        para: data of index row in mcdat.
...
def plot_displacement(x, y, z):
    """
    Plot displacement of head motion.

    Parameters
    ----------
        x: dS, displacement in the Superior direction (mm)
        y: dL, displacement in the Left direction (mm)
        z: dP, displacement in the Posterior direction (mm)
    """
...
def plot_rotation(a, b, c):
    """
    Plot rotation of head motion.

    Parameters
    ----------
        a: roll, rotation about the I-S axis (degrees CCW)
        b: pitch, rotation about the R-L axis (degrees CCW)
        c: yaw, rotation about the A-P axis (degrees CCW)
    """
